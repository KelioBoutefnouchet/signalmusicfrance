(() => {
  ...
  const setState = (label, state) => {
    ...
    player.classList.toggle('is-off', state === 'off');
    toggle.setAttribute(
      'aria-label',
      state === 'playing' ? 'Mettre la radio en pause' : 'Lire la radio'
    );
  };

  const iframe = document.createElement('iframe');
  iframe.id = 'signal-soundcloud-widget';
  iframe.className = 'soundcloud-widget';
  iframe.title = 'Lecteur SoundCloud Signal Music France';
  ...
  const initialise = () => {
    ...
    widget.bind(window.SC.Widget.Events.PLAY, () => {
      ...
      widget.getCurrentSound((sound) =>
        setState(`PLAYING — ${sound?.title || 'SIGNAL RADIO'}`, 'playing')
      );
    });
  ...
  api.async = true;
  api.onload = initialise;
  api.onerror = () => setState('OFF AIR', 'off');
